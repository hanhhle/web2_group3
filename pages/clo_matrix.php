...
function pageCLO_MATRIX(): void {
    ...
    $selProg  = (int)($_GET['program_id'] ?? 0);
    $selSubj  = (int)($_GET['subject_id'] ?? 0);
    $subjsOfProg = $selProg ? qAll("SELECT * FROM subjects WHERE program_id=? ORDER BY code",[$selProg]) : [];
    $plos = $selProg ? qAll("SELECT * FROM plos WHERE program_id=? ORDER BY code",[$selProg]) : [];
    $clos = $selSubj ? qAll("SELECT * FROM clos WHERE subject_id=? ORDER BY code",[$selSubj]) : [];
    $existing = [];

    if ($selSubj) {
        foreach (qAll("SELECT clo_id,plo_id FROM clo_plo_mappings WHERE clo_id IN (SELECT id FROM clos WHERE subject_id=?)",[$selSubj]) as $r)
            $existing[$r['clo_id']][$r['plo_id']] = true;
    }
