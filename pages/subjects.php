...
function pageSUBJECTS(): void {
    ...
    $subjects = qAll("SELECT s.*, p.name pn, p.code pc, COUNT(DISTINCT c.id) clo_count, COUNT(DISTINCT a.id) assess_count FROM subjects s JOIN programs p ON s.program_id=p.id LEFT JOIN clos c ON s.id=c.subject_id LEFT JOIN assessments a ON s.id=a.subject_id GROUP BY s.id ORDER BY p.code, s.code");
    $programs = qAll("SELECT * FROM programs ORDER BY code");
    $edit = isset($_GET['edit']) ? qOne("SELECT * FROM subjects WHERE id=?", [(int)$_GET['edit']]) : null;
    $isEdit = (bool)$edit; 
    ...
    echo "<div class='col-md-3'><label class='form-label academic-field-label'>Program Route *</label><select name='program_id' class='form-select academic-field-select' required><option value=''>-- Select --</option>";
    ...
    echo "<div class='col-md-4'><label class='form-label academic-field-label'>Subject Name *</label><input type='text' name='name' class='form-control academic-field-input' placeholder='e.g. Web Development' value='" . h($edit['name'] ?? '') . "' required></div>";
    echo "<div class='col-md-1'><label class='form-label academic-field-label'>Credits *</label><input type='number' name='credits' class='form-control academic-field-input' min='1' max='10' value='" . ($edit['credits'] ?? 3) . "' required></div>";
    ...
    foreach ($subjects as $s) {
        ...
        // Tên môn học chính quy
        echo "<td class='fw-semibold' style='color: #0b1320; font-size: 0.95rem;'>" . h($s['name']) . "</td>";
