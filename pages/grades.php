...
function pageGRADES(): void {
    ...
    $u = uid();
    $enrollments = qAll("SELECT se.*,s.name sn,s.code sc,s.credits FROM student_enrollments se JOIN subjects s ON se.subject_id=s.id WHERE se.student_id=? ORDER BY se.semester DESC,s.code",[$u]);
    ...
    foreach ($enrollments as $enr) {
        $assessments = qAll("SELECT a.*,r.id rid,r.name rn,u.full_name as lecturer_name FROM assessments a LEFT JOIN rubrics r ON a.id=r.assessment_id LEFT JOIN users u ON a.lecturer_id=u.id WHERE a.subject_id=? AND a.semester=? ORDER BY FIELD(a.type, 'attendance', 'midterm', 'final')",[$enr['subject_id'],$enr['semester']]);
        ...
        if (empty($assessments)) {
            ...
            foreach ($assessments as $a) {
                ...
                if ($a['rid']) {
                    $criteria = qAll(
                        "SELECT rc.*, ss.score, ss.graded_at, u.full_name as grader_name, u.username as grader_username, c.code clo_code
                         ...
                         WHERE rc.rubric_id=? ORDER BY rc.id",
                        [$enr['id'], $a['rid']]
                    );
                }
