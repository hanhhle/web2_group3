...
function pageENROLLMENTS(): void {
    ...
    $enrollments = qAll("SELECT se.*, u.full_name sn, u.username un, s.name subjn, s.code subjc FROM student_enrollments se JOIN users u ON se.student_id=u.id JOIN subjects s ON se.subject_id=s.id ORDER BY se.semester DESC, u.full_name");
    $students = qAll("SELECT * FROM users WHERE role='student' ORDER BY full_name");
    $subjects = qAll("SELECT * FROM subjects ORDER BY code");
