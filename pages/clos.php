...
function pageCLOS(): void {
    ...
    $clos = qAll("SELECT c.*,s.name sn,s.code sc,COUNT(DISTINCT m.id) map_count FROM clos c JOIN subjects s ON c.subject_id=s.id LEFT JOIN clo_plo_mappings m ON c.id=m.clo_id GROUP BY c.id ORDER BY s.code,c.code");
    $subjects = qAll("SELECT * FROM subjects ORDER BY code");
    $edit = isset($_GET['edit']) ? qOne("SELECT * FROM clos WHERE id=?",[(int)$_GET['edit']]) : null;
    $isEdit = (bool)$edit; 
