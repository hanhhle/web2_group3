...
function pageGRADE_REQUESTS(array $allReqs = []): void {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.ajax-request-form').forEach(form => {
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    if(data.success) {
                        alert(data.message); // Có thể thay bằng Toast nếu bạn muốn
                        // Xóa mượt mà thẻ chứa request mà KHÔNG F5
                        const wrapperId = 'request-wrapper-' + form.querySelector('input[name=\"request_id\"]').value;
                        const cardElement = document.getElementById(wrapperId).closest('.req-card');
                        if (cardElement) {
                            cardElement.style.transition = 'all 0.5s ease';
                            cardElement.style.opacity = '0';
                            cardElement.style.transform = 'scale(0.95)';
                            setTimeout(() => cardElement.remove(), 500);
                            document.querySelectorAll('.badge, .nav-link-custom span, .sidebar-link span').forEach(badge => {
                            let currentCount = parseInt(badge.innerText);
                            if (!isNaN(currentCount) && currentCount > 0 && badge.innerText.trim() === currentCount.toString()) {
                                currentCount--; // Trừ đi 1
                                if (currentCount === 0) {
...
function renderRequestCard(array $r, bool $showActions): void {
    ...
    if ($showActions) {
        ...
        // Bao bọc toàn bộ form bằng 1 thẻ div có ID để xóa khi duyệt xong
        echo "<div id='request-wrapper-" . $r['id'] . "' style='display:inline;'>";
        
        // Approve form
        echo "<form class='ajax-request-form d-inline' data-confirm='Approve this grade change request?'>
                <input type='hidden' name='csrf_token' value='{$csrf}'>
                <input type='hidden' name='_action' value='approve_grade_change_ajax'>
        ...
        // Reject form
        echo "<form class='ajax-request-form d-inline' data-confirm='Reject this grade change request?'>
                <input type='hidden' name='csrf_token' value='{$csrf}'>
                <input type='hidden' name='_action' value='reject_grade_change_ajax'>
                <input type='hidden' name='request_id' value='{$r['id']}'>
